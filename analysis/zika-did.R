...
### Placebo in-time
# Conduct the same analysis for each pair of years on the state level and plot the estimates
...
### Log-scale model
zika_log_lm <- lm(LogRate~State+year+interaction, data=zika_full)
summary(zika_log_lm)
...
log_boot <- replicate(n=1000,
                      expr=lm(LogRate~State+year+interaction,
                              data=boot_data())$coefficients["interaction"])
