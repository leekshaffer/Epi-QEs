...
Note that the static covariate does not affect the results, as there are already fixed effects included for each state. The two different models (no covariates vs. time-varying covariate) give diverging results, with fairly large confidence intervals, leading to no clear conclusion. The estimates are hard to interpret as well given the changing circumstances of the states involved.
...
This is the basic dynamic estimator that arises from including specifying leads/lags in the TWFE model. You can see some evidence for heterogeneous treatment effects: the effect seems to grow with time since treatment. There is also evidence for non-parallel trends: the lead times consistently have positive point estimates, indicating the treated states were growing faster than untreated states prior to treatment. These are decreasing, indicating that this effect was disappearing, or possibly changing.
...
In this case, the results are quite similar to the TWFE event study specification. Since there are so many untreated states, limiting comparisons to not-yet-treated does not remove many comparisons.
...
Similarly, the Sun and Abraham estimator that only uses never-treated for comparison has similar results as well.
...
This approach gives noticeably different results from the others, especially in the lead periods. This may be because the CS specification allows not-yet-treated controls to be used and later-treated units may have better parallel trends than never-treated units. CS also does not require comparison only to the last-pretreated period, allowing more information to be used in the adjustment for fixed effects of time.
...
```{r}
## It requires a 0 value for the start period for untreated units
Vax_CS <- Vax_adj %>% dplyr::filter(Yr_Wk %in% Yr_Wk_Sel) %>%
                    left_join(Vax_weekly %>% dplyr::filter(LeadLag==0) %>% 
                                dplyr::select(State,MMWR_week) %>%
                                rename(First_Week=MMWR_week),
                              by=join_by(State)) %>%
                    left_join(tibble(State=unique(Vax_weekly %>% pull(State))) %>%
                                ...
                                mutate(StateID=1:length(unique(Vax_weekly %>% pull(State)))),
                              by=join_by(State)) %>%
  dplyr::mutate(First_Week=if_else(is.na(First_Week),0,First_Week))
```
...
Other methods, specifications, and aggregations are possible. The review papers listed in the slides are a good place to start to investigate these.

## Discussion Questions

- Are this question and its associated data set a good fit for a DID/staggered adoption analysis? Why or why not?
