...
chol_2x2 <- regdata %>% 
  mutate(Supplier=factor(supplier,
                         ...
                         labels=c("Joint Southwark & Vauxhall/Lambert (Treated)",
...
diffs <- chol_2x2_f %>%
  dplyr::filter(Supplier=="Joint Southwark & Vauxhall/Lambert (Treated)") %>% 
...
```{r}
POs <- rep(chol_2x2 %>% dplyr::filter(Supplier=="Joint Southwark & Vauxhall/Lambert (Treated)",
                                      year==1849) %>% 
             pull(mortality),2) + 
  c(0,chol_2x2_res %>% 
      dplyr::filter(Supplier=="Southwark & Vauxhall Only (Untreated)") %>% 
...
chol_2x2_po <- chol_2x2 %>% dplyr::select(-c(N)) %>%
  ...
  bind_rows(tibble(Supplier="Joint Southwark & Vauxhall/Lambert (Treated)",
                   year=c("1849","1854"),
                   mortality=POs,
                   Type="Counterfactual")) %>%
  mutate(Type=factor(Type,levels=c("Observed","Counterfactual")),
         Supplier=factor(Supplier,levels=c("Joint Southwark & Vauxhall/Lambert (Treated)",
                                           "Southwark & Vauxhall Only (Untreated)"),
                         labels=c("Treated","Untreated")),
         group=paste0(Type,", ",Supplier))
ggplot(chol_2x2_po) + 
  geom_line(mapping=aes(x=year, y=mortality, color=Supplier, linetype=Type, group=group),
linewidth=1.5) + 
  theme_bw() +
