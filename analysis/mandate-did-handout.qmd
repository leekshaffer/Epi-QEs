...
3.  `DIDmultiplegt` and `DIDmultiplegtDYN` implement the first-difference approach of [de Chaisemartin and d'Haultfoeuille (2020)](https://doi.org/10.1257/aer.20181169) and [de Chaisemartin and d'Haultfoeuille (2023)](https://doi.org/10.1093/ectj/utac017).
...
# install.packages("bacondecomp","did2s","DIDmultiplegt,"DIDmultiplegtDYN")
require(tidyverse)
require(knitr)
require(lme4)
require(bacondecomp)
require(did2s)
## If only DIDmultiplegt can be installed for you, only use that one:
...
If you were able to install the `DIDmultiplegtDYN` package, a faster analytical implementation is available through the `did_multiplegtDYN` function.
